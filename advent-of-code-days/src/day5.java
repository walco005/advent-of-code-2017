...
//			i += j;
//			count++;
//			System.out.println("At the " + count + "th step, we are at index " + i);
//		}
		System.out.println("Yay!! We escaped at the " + count + "th step!! We're finally free!! :)");
		
		//Part Two
		int x = 0;
		int y;
		int count2 = 0;
		while(x >= 0 && x < input.length) {
			y = input[x];
			if(y >= 3) {
				input[x]--;
			} else {
			...
			x += y;
			count2++;
			System.out.println("At the " + count2 + "th step, we are at index " + x);
		}
	}
